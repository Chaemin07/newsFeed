...
public class LikeService {
    ...
    public long likeCounter(LikeType type, long id) {
        ...
        switch(type) {
            case NEWSFEED -> {
                return newsFeedLikeRepository.countByNewsFeed_FeedId(id);
            }
            case COMMENT -> {
                return commentLikeRepository.countByComment_ParentId(id);
            }
        ...
        switch(type) {
            case NEWSFEED:
                ...
                NewsFeed newsFeed = newsFeedRepository.findById(likeId)
                        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "게시글을 찾을 수 없습니다."));
                if(newsFeed.getCreator().getId().equals(userId)) {
